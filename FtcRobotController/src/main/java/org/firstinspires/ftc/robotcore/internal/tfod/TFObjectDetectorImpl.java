  ...
  private static int getRotation(Activity activity, CameraName cameraName) {
    ...
    if (cameraName instanceof BuiltinCameraName) {
      ...
      for (int cameraId = 0; cameraId < Camera.getNumberOfCameras(); cameraId++) {
        ...
        if (cameraInfo.facing == CameraInfo.CAMERA_FACING_FRONT && cameraDirection == CameraDirection.FRONT) {
          rotation = - displayRotation - cameraInfo.orientation;
          break;
        }
        if (cameraInfo.facing == CameraInfo.CAMERA_FACING_BACK && cameraDirection == CameraDirection.BACK) {
          rotation = displayRotation - cameraInfo.orientation;
          break;
        }
      }
